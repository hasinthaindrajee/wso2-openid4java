...
<%@ page contentType="text/html; charset=UTF-8" import="java.util.LinkedHashMap,java.util.Map" %>
...
<div>
    ...
    <fieldset>
        <legend>Sample 2: using the Simple Registration extension(doc: <a
                href="http://code.google.com/p/openid4java/wiki/SRegHowTo">SRegHowTo</a>)
        </legend>
        <form action="consumer" method="post">
            <div id="sreg">
                <input type="text" name="openid_identifier"/>
                ...
                <table>
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="radio" name="all" value="0" id="all0" onclick="changeAll('0')"/><label
                                for="all0">Optional</label>
                            <input type="radio" name="all" value="1" id="all1" onclick="changeAll('1')"/><label
                                for="all1">Required</label>
                        </td>
                    </tr>
                    <%
                    ...
                    <c:forEach items="<%=sRegAttrs %>" var="attribute">
                        <tr>
                            ...
                            <td>
                                <input type="radio" name="${attribute.key}" value="" id="${attribute.key}"
                                       checked="checked"/><label for="${attribute.key}">None</label>
                                <input type="radio" name="${attribute.key}" value="0" id="${attribute.key}0"/><label
                                    for="${attribute.key}0">Optional</label>
                                <input type="radio" name="${attribute.key}" value="1" id="${attribute.key}1"/><label
                                    for="${attribute.key}1">Required</label>
                            </td>
                        </tr>
                    </c:forEach>
                </table>
    ...
    <fieldset>
        <legend>Sample 3: using the Attribute Exchange extension(doc: <a
                href="http://code.google.com/p/openid4java/wiki/AttributeExchangeHowTo">AttributeExchangeHowTo</a>)
        </legend>
        <form action="consumer" method="post">
            <div id="ax">
                ...
                <table>
                    ...
                    <c:forEach items="<%=attributes%>" var="attribute" varStatus="status">
                        <tr>
                            ...
                            <td><input type="checkbox" name="required${status.index}" id="required${status.index}"
                                       checked="checked"/><label for="required${status.index}">Required</label></td>

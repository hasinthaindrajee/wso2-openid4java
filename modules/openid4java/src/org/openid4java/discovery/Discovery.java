...
public class Discovery {
    ...
    public static XriResolver getXriResolver() {
        ...
        if (DEBUG) {
            _log.debug(XRI_RESOLVER_CLASS_NAME_KEY + ":" + className);
        }
        try {
    ...
    public Identifier parseIdentifier(String identifier,
        ...
        try {
            ...
            if (identifier.toLowerCase().startsWith("xri://")) {
                if (DEBUG) {
                    _log.debug("Dropping xri:// prefix from identifier: "
                               + identifier);
                }
            ...
            if (URL_PATTERN.matcher(identifier).find()) {
                if (DEBUG) {
                    _log.debug("Creating URL identifier for: " + identifier);
                }
                return new UrlIdentifier(identifier, removeFragment);
            } else if (XRI_PATTERN.matcher(identifier).find()) {
                if (DEBUG) {
                    _log.debug("Creating XRI identifier for: " + identifier);
                }
                return _xriResolver.parseIdentifier(identifier);
            } else {
                if (DEBUG) {
                    _log.debug("Creating URL identifier (http:// prepended) for: "
                               + identifier);
                }
